...
test('API profile photos returns tweets and cursors for known user', async () => {
  const result = await app.request(
    new Request('https://api.fxtwitter.com/2/profile/x/photos', {
      method: 'GET',
      headers: botHeaders
    }),
    undefined,
...
test('API profile photos accepts cursor query param', async () => {
  const result = await app.request(
    new Request(
      'https://api.fxtwitter.com/2/profile/x/photos?cursor=DAAHCgABHEJR3hu___QLAAIAAAATMjAyODI4MTMx',
      {
        method: 'GET',
        headers: botHeaders
      }
...
test('API profile photos returns 404 for unknown user', async () => {
  const result = await app.request(
    new Request('https://api.fxtwitter.com/2/profile/__no_such_user_fxt__/photos', {
      method: 'GET',
      headers: botHeaders
    }),
    undefined,
