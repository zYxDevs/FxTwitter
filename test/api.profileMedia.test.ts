...
test('API profile media returns tweets and cursors for known user', async () => {
  const result = await app.request(
    new Request('https://api.fxtwitter.com/2/profile/x/media', {
      method: 'GET',
      headers: botHeaders
    }),
    undefined,
...
test('API profile media accepts count and cursor query params', async () => {
  const result = await app.request(
    new Request(
      'https://api.fxtwitter.com/2/profile/x/media?count=5&cursor=DAAHCgABHEJR3hu___QLAAIAAAATMjAyODI4MTMx',
      {
        method: 'GET',
        headers: botHeaders
      }
...
test('API profile media returns 404 for unknown user', async () => {
  const result = await app.request(
    new Request('https://api.fxtwitter.com/2/profile/__no_such_user_fxt__/media', {
      method: 'GET',
      headers: botHeaders
    }),
    undefined,
